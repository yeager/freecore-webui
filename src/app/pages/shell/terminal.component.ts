...
import { TERMINAL_FONT_FAMILY, whenTerminalFontReady } from './terminal-font';
...
export class TerminalComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    // xterm measures its cell at open() and never again unless the font
    // options change, and the Plex Mono webfont loads lazily — so open only
    // once it is in (bounded wait), or measure and render disagree (#357).
    whenTerminalFontReady(this.fontSize).then(() => {
      if (this.destroyed) {
        return;
      }
      this.zone.runOutsideAngular(() => this.openTerminal());
