...
import {
  UntypedFormGroup, Validators, ValidationErrors, UntypedFormControl,
} from '@angular/forms';
...
export class VMWizardComponent {
  ...
  afterInit(entityWizard: EntityWizardComponent) {
    ...
    this.ws.call('vm.device.vnc_bind_choices').subscribe((res) => {
      if (res && Object.keys(res).length > 0) {
        ...
        (< UntypedFormGroup > entityWizard.formArray.get([0]).get('vnc_bind')).setValue(res['0.0.0.0']);
      }
    });
    ...
    (< UntypedFormGroup > entityWizard.formArray.get([0]).get('bootloader')).valueChanges.subscribe((bootloader) => {
    ...
    (< UntypedFormGroup > entityWizard.formArray.get([0]).get('enable_vnc')).valueChanges.subscribe((res) => {
      ...
      if (res) {
        (< UntypedFormGroup > entityWizard.formArray.get([0]).get('wait')).enable();
        (< UntypedFormGroup > entityWizard.formArray.get([0]).get('vnc_bind')).enable();
      } else {
        (< UntypedFormGroup > entityWizard.formArray.get([0]).get('wait')).disable();
        (< UntypedFormGroup > entityWizard.formArray.get([0]).get('vnc_bind')).disable();
      }
    });

    (< UntypedFormGroup > entityWizard.formArray.get([0]).get('os')).valueChanges.subscribe((res) => {
      this.summary[T('Guest Operating System')] = res;
      (< UntypedFormGroup > entityWizard.formArray.get([0])).get('name').valueChanges.subscribe((name) => {
        this.summary[T('Name')] = name;
      });
      (< UntypedFormGroup > entityWizard.formArray.get([1])).get('vcpus').valueChanges.subscribe((vcpus) => {
        this.vcpus = vcpus;
        this.summary[T('Number of CPUs')] = vcpus;
      });
      (< UntypedFormGroup > entityWizard.formArray.get([1])).get('cores').valueChanges.subscribe((cores) => {
        this.cores = cores;
        this.summary[T('Number of Cores')] = cores;
      });
      (< UntypedFormGroup > entityWizard.formArray.get([1])).get('threads').valueChanges.subscribe((threads) => {
        this.threads = threads;
        this.summary[T('Number of Threads')] = threads;
      });
      (< UntypedFormGroup > entityWizard.formArray.get([1])).get('memory').valueChanges.subscribe((memory) => {
        this.summary[T('Memory')] = isNaN(this.storageService.convertHumanStringToNum(memory))
          ? '0 MiB'
          : this.storageService.humanReadable;
      });

      (< UntypedFormGroup > entityWizard.formArray.get([2])).get('volsize').valueChanges.subscribe((volsize) => {
        this.summary[T('Disk Size')] = volsize;
      });

      (< UntypedFormGroup > entityWizard.formArray.get([2])).get('disk_radio').valueChanges.subscribe((disk_radio) => {
        if (this.summary[T('Disk')] || this.summary[T('Disk Size')]) {
          delete this.summary[T('Disk')];
          delete this.summary[T('Disk Size')];
        }
        if (disk_radio) {
          this.summary[T('Disk Size')] = (< UntypedFormGroup > entityWizard.formArray.get([2])).controls['volsize'].value;
          (< UntypedFormGroup > entityWizard.formArray.get([2])).get('volsize').valueChanges.subscribe((volsize) => {
            this.summary[T('Disk Size')] = volsize;
          });
        } else {
          this.summary[T('Disk')] = (< UntypedFormGroup > entityWizard.formArray.get([2])).controls['hdd_path'].value;
          (< UntypedFormGroup > entityWizard.formArray.get([2])).get('hdd_path').valueChanges.subscribe((existing_hdd_path) => {
            this.summary[T('Disk')] = existing_hdd_path;
          });
        }
      });

      (< UntypedFormGroup > entityWizard.formArray.get([2])).get('datastore').valueChanges.subscribe((datastore) => {
        if (datastore !== undefined && datastore !== '' && datastore !== '/mnt') {
          ...
          const volsize = this.storageService.convertHumanStringToNum((< UntypedFormGroup > entityWizard.formArray.get([2])).controls['volsize'].value.toString());
          this.ws.call('filesystem.statfs', [`/mnt/${datastore}`]).subscribe((stat) => {
            ...
            if (stat.free_bytes < volsize) {
              (< UntypedFormGroup > entityWizard.formArray.get([2])).controls['volsize'].setValue(Math.floor(stat.free_bytes / (1073741824)));
            } else if (stat.free_bytes > 40 * 1073741824) {
              const vm_os = (< UntypedFormGroup > entityWizard.formArray.get([0]).get('os')).value;
              if (vm_os === 'Windows') {
                (< UntypedFormGroup > entityWizard.formArray.get([2])).controls['volsize'].setValue(this.storageService.convertBytestoHumanReadable(volsize, 0));
              } else {
                (< UntypedFormGroup > entityWizard.formArray.get([2])).controls['volsize'].setValue(this.storageService.convertBytestoHumanReadable(volsize, 0));
              }
          ...
          if (datastore === '/mnt') {
            (< UntypedFormGroup > entityWizard.formArray.get([2])).controls['datastore'].setValue(null);
      ...
      (< UntypedFormGroup > entityWizard.formArray.get([3]).get('NIC_type')).valueChanges.subscribe((res) => {
        this.prefService.preferences.nicType = res;
      });
      (< UntypedFormGroup > entityWizard.formArray.get([3]).get('nic_attach')).valueChanges.subscribe((res) => {
        this.prefService.preferences.nicAttach = res;
      });
      (< UntypedFormGroup > entityWizard.formArray.get([4]).get('iso_path')).valueChanges.subscribe((iso_path) => {
      ...
      this.messageService.messageSourceHasNewMessage$.subscribe((message) => {
        (< UntypedFormGroup > entityWizard.formArray.get([4]).get('iso_path')).setValue(message);
      });
      this.res = res;
      if (res === 'Windows') {
        (< UntypedFormGroup > entityWizard.formArray.get([1])).controls['vcpus'].setValue(2);
        (< UntypedFormGroup > entityWizard.formArray.get([1])).controls['cores'].setValue(1);
        (< UntypedFormGroup > entityWizard.formArray.get([1])).controls['threads'].setValue(1);
        (< UntypedFormGroup > entityWizard.formArray.get([1])).controls['memory'].setValue('4 GiB');
        (< UntypedFormGroup > entityWizard.formArray.get([2])).controls['volsize'].setValue('40 GiB');
      } else {
        (< UntypedFormGroup > entityWizard.formArray.get([1])).controls['vcpus'].setValue(1);
        (< UntypedFormGroup > entityWizard.formArray.get([1])).controls['cores'].setValue(1);
        (< UntypedFormGroup > entityWizard.formArray.get([1])).controls['threads'].setValue(1);
        (< UntypedFormGroup > entityWizard.formArray.get([1])).controls['memory'].setValue('512 MiB');
        (< UntypedFormGroup > entityWizard.formArray.get([2])).controls['volsize'].setValue('10 GiB');
      }
    });
    (< UntypedFormGroup > entityWizard.formArray.get([2]).get('disk_radio')).valueChanges.subscribe((res) => {
      if (res) {
    ...
    (< UntypedFormGroup > entityWizard.formArray.get([4]).get('upload_iso_checkbox')).valueChanges.subscribe((res) => {
      if (res) {
    ...
    (< UntypedFormGroup > entityWizard.formArray.get([4]).get('upload_iso_path')).valueChanges.subscribe((res) => {
      if (res) {
    ...
    this.networkService.getVmNicChoices().subscribe((res) => {
      ...
      const tempNICAttach = (< UntypedFormGroup > entityWizard.formArray.get([3])).controls['nic_attach'];
      setTimeout(() => {
      ...
      this.ws.call('vm.random_mac').subscribe((mac_res) => {
        (< UntypedFormGroup > entityWizard.formArray.get([3])).controls['NIC_mac'].setValue(mac_res);
      });
    });
    ...
    const tempNICType = (< UntypedFormGroup > entityWizard.formArray.get([3])).controls['NIC_type'];
    setTimeout(() => {
    ...
    (< UntypedFormGroup > entityWizard.formArray.get([0])).controls['bootloader'].setValue(
  ...
  memoryValidator(name: string) {
    const self = this;
    return function validMem(control: UntypedFormControl) {
  ...
  volSizeValidator(name: string) {
    const self = this;
    return function validStorage(control: UntypedFormControl) {
